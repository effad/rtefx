...
public class REDSyntaxHighlighterDefinition implements REDXMLReadable {
	...
	public void innerProduction(REDXMLReadable obj, REDXMLHandlerReader inner, REDXMLHandlerReader outer) {
		if (obj instanceof REDSyntaxHighlighterRule) {
			REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterDefinition", "Adding rule to SHD: " + obj);
			addRule((REDSyntaxHighlighterRule) obj);
		}
		else {
			REDTracer.warning("org.rtefx.plugins.synHi", "REDSyntaxHighlighterDefinition", "Unknown inner production ignored: " + obj);
		}
	}

	public void setName(String name) {
		fName = name;
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterDefinition", "SHD name set to: " + name);
	}
