...
public class REDSyntaxHighlighterManager {
	...
	static private void readHighlighters() {
		...
		while (iter.hasNext()) {
			...
			try {
				manager.parse(reader, "" + iter.curName());
				REDSyntaxHighlighterDefinition def = (REDSyntaxHighlighterDefinition) manager.getProducedObject();
				if (def != null) {
					REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterManager", "Added SHD '" + def.getName() +"'");
					addDefinition(def);
				}
				else {
					REDTracer.error("org.rtefx.plugins.synHi", "REDSyntaxHighlighterManager", "Error while reading SHD from '" + iter.curName() +"'.");
				}
			}
			catch (Exception e) {
				REDTracer.error("org.rtefx.plugins.synHi", "REDSyntaxHighlighterManager", "Exception while reading SHD from '" + iter.curName() +"': "+ e);
			}
			try { reader.close(); } catch (IOException ioe) { }
		}
	}
	
	private static HashMap fDefinitions = new HashMap();
	static {
		readHighlighters();
	}
}
