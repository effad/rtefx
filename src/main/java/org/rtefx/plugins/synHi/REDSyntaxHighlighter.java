...
public class REDSyntaxHighlighter extends REDPlugin implements REDRexParserStopper {
	public REDSyntaxHighlighter(REDSyntaxHighlighterDefinition def) {
		...
		while (iter.hasNext()) {
			...
			try {
				r.installInParser(fParser.defaultState(), def.getCaseSensitive(), fParser, REDStyleManager.getDefaultStyle());
			}
		...
		try {
			wordIgnorer.installInParser(fParser.defaultState(), def.getCaseSensitive(), fParser, REDStyleManager.getDefaultStyle());
		}
		catch (REDRexMalformedPatternException mpe) {
...
//			REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter", "State for line " + i + " is " + getState(i));
//		}
//	}
	
	public boolean mustStop(REDRexParser parser, int line, int offset, int state) {		
//		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter", "mustStop called for line: " + line + " with state: " + state + " and getState() == " + getState(line) + ", offset = " + offset);
		if (fRestartFromLine != -1) {
			return true;
		}
		boolean retVal = !fUpdateAll && line > fLastParsedLine && state == getState(line);
		if (!retVal && offset == 0) {
//			REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter",  "Setting state for line " + line + " to " + state);
			setState(line, state);
		}
//		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter",  "mustStop returns: " + retVal);
	...
	class ConcurrentParser extends Thread {
		...
		public void run() {
			do {
				...
				fParser.putClientProperty("changeCount", new Integer(fChangeCount));
	...
	synchronized public void waitForParser() throws InterruptedException {
		if (fConcurrentParser != null) {
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() { public void run() { } });	// Wait for batch executor ...
			}
...
//		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter", "updateLines entered for line = " + line + ", all = " + all);		
		fUpdateAll = all;
		if (fUpdateAll) 
		{
		...
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter", "Syntax Highlighter took " + (end - start) + " msec.");
	}
...
//		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter", "deleteStates: " + (fEditor.getLineForPosition(from) + 1) + " - " + fEditor.getLineForPosition(to));
...
//		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighter", "Creating new mark tree.");
		fChangeCount++;
