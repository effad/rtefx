	...
	public void innerProduction(REDXMLReadable obj, REDXMLHandlerReader inner, REDXMLHandlerReader outer) {
		if (outer.peekClientData("subparsers") != null) {
			if (obj instanceof REDSyntaxHighlighterRule) {
				...
				REDTracer.warning("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "Unknown inner production ignored: " + obj);
			}				
		}
		else {
			REDTracer.warning("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "Inner production ignored, because subparser tag not encountered: " + obj);
		}
	}

	
	public void setStart(String start) {
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "Start RegExp set for range: " + start);
		fStart = start;
	}
	
	public void setEnd(String end) {
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "End RegExp set for range: " + end);
		fEnd = end;
	}
	
	public void setRewind() {
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "Rewind turned on for range.");
		fRewind = true;
	}
	
	public void subparsersOn(REDXMLHandlerReader handler) {
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "Subparsers turned on for range.");
		handler.pushClientData("subparsers", new Boolean(true));
	}
	
	public void subparsersOff(REDXMLHandlerReader handler) {
		REDTracer.info("org.rtefx.plugins.synHi", "REDSyntaxHighlighterRange", "Subparsers turned off for range.");
