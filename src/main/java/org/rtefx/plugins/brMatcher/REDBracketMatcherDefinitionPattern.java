...
public class REDBracketMatcherDefinitionPattern implements REDXMLReadable {
	...
	boolean isOk() {
		return fLeft != null && !fLeft.equals("") &&
			fRight != null && !fRight.equals("") &&
			fStyleSet != null && fRuleSet != null;
	}

	String mirrorString(String str) {
	...
	private boolean isIgnoredStyle(REDStyle style) {
		...
		while (iter.hasNext()) {
			if (style.isA(REDStyleManager.getStyle("" + iter.next()))) {
				return true;
			}
		}
		return false;
	...
	private boolean bufEquals(char [] buf, int idx, String str) {
