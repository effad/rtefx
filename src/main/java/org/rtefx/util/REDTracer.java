...
final public class REDTracer implements REDXMLReadable {
	...
	private static void readConfigs() {
		...
		while (iter.hasNext()) {
			...
			readConfig(is, "" + iter.curName());
		}
	}
	
	private String fPackage;	
	static int fgLevel;
	private static HashMap fgPackages = new HashMap();	// map of package name to ArrayList of Rules
	private static HashSet fgWarnedPacks = new HashSet();
	private static boolean fgDefault = false;
	private static int fgWantLevel = 0;	// auxiliary variable to prevent endless recursion
