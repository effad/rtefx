...
public class REDRexParser {
	...
	private boolean isPositionOK(REDRexParserMatch m, int minReq, boolean reverse) {
		if (reverse) {
	...
	private int getStartReqMinPos(boolean reverse, REDRexLineSource src, int line) {
		if (reverse) {
	...
	private int getReqMinPos(boolean reverse, REDRexParserMatch m) {
		if (reverse) {
			return m.getStart(0);
		}
		else {
