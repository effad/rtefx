...
//		v.addMouseMotionListener(this);
	...
	/**
	  * @pre v != null
	  */
	boolean needTypingCmd(REDView v, String cmdType) {
	...
	private boolean atLineStart(REDText text, int pos) {
	...
	public void key(REDView v, KeyEvent e) {
		...
		if (needTypingCmd(v, "Typing")) {
			if (v.hasSelection()) {
				cmd = new REDTextCommand ("Replace", v, text, selStart, selEnd-selStart, "" + e.getKeyChar());
			}
			else {
				if (v.getMode() == REDAuxiliary.VIEWMODE_INSERT || selStart == text.length() || selStart == text.getLineEnd(text.getLineForPosition(selStart))) {	
					cmd = new REDTextCommand ("Typing", v, text, selStart, 0, "" + e.getKeyChar());
				}
				else {
					cmd = new REDTextCommand("Replace", v, text, selStart, 1, "" + e.getKeyChar());
				}
			}
			text.setCurTypingCommand(cmd);
			text.getCommandProcessor().perform(cmd);		
			...
			text.replace(selStart, selStart, "" + e.getKeyChar());	
			cmd.addChar();		
			v.setSelection(++selStart);
		}
		selStart = v.fSelFrom; 
