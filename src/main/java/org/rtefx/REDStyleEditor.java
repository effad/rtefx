...
public class REDStyleEditor implements TreeSelectionListener, REDStyleEditorSelectionListener  {
	...
	private void setupThemeChooser() {
		...
		fThemes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				fStyleManager.doSetTheme("" + fThemes.getSelectedItem());				
				updateHierarchyGUI();
	...
	class ButtonColorizer implements ActionListener {
	...
	private void setupStyleEditGUIWidgetsFontFace() {
		...
		fEditFontFace.addActionListener(new StyleEditListener() {
			void stateChanged(REDStyle s) {
				if (! ("".equals(fEditFontFace.getSelectedItem()) || s.getFontFace().equals("" + fEditFontFace.getSelectedItem()))) {
					s.setFontFace(getTheme(), "" + fEditFontFace.getSelectedItem());
				}
			}
		});
	...
	private void setupStyleEditGUIWidgetsFontSize() {
		...
		fEditFontSize.addActionListener(new StyleEditListener() {
			void stateChanged(REDStyle s) {
				try {
					int x = Integer.parseInt("" + fEditFontSize.getEditor().getItem());
		...
		fEditFontSizeInherited.addActionListener(new StyleEditListener() {
			void stateChanged(REDStyle s) {
				if (fEditFontSizeInherited.isSelected()) {
					...
					int x = s.getFontSize();
					try {
						x = Integer.parseInt("" + fEditFontSize.getEditor().getItem());
					}
					catch (NumberFormatException nfe) { }
					s.setFontSize(getTheme(), x);
				}
	...
	private void setupStyleEditGUIWidgetsFontStyle() {
		...
		fEditFontStyle.addActionListener(new StyleEditListener() {
			void stateChanged(REDStyle s) {
				if (! ("".equals(fEditFontStyle.getSelectedItem()) || s.getFontStyle().equals("" + fEditFontStyle.getSelectedItem()))) {
					s.setFontStyle(getTheme(), "" + fEditFontStyle.getSelectedItem());
				}
			}
		});
	...
	private GridBagConstraints createConstraints(double weightx, int gridwidth, int anchor, int fill) {
	...
	public void updateStyleEditGUI(REDStyle style) {
		if (style == null) {
			...
			fEditFontSize.getEditor().setItem("" + style.getFontSize(theme));
	...
	public String getTheme() {
		return "" + fThemes.getSelectedItem();
	}
