...
public class REDFinder implements REDRexLineSource {
	...
	/** Quote characters in pattern with backslash
	  ...
	  */
	String quoteWithBackslash(String pattern, String tbQuoted) {
	...
	private class OneMatchStopper extends REDRexAction implements REDRexParserStopper {
	...
	public void find(String pattern, int from, REDFinderDirection direction, boolean caseSensitive, 
		...
		fParser.addRule(fParser.defaultState(), pattern, caseSensitive, fParser.defaultState(), this, action, false);
		fParser.parse(this, 0, fParser.defaultState(), stopper, fParseReverse);
	}
	...
	private class ReplaceAction extends REDRexAction {
		...
		private int executeReplacements(boolean withinSelection) {
			...
			while (iter.hasNext()) {
				...
				from = getRealPosition(e.fLine, e.fDetails.getStart(0));
				to = getRealPosition(e.fLine, e.fDetails.getEnd(0));
				if (fReplacement.indexOf("\\") != -1) {	// we have backrefs
	...
	private REDRexParser fParser;
	private int fParseStart, fParseStartLine;
	private boolean fParseReverse;
	private DefaultComboBoxModel fFindHistory;
	private DefaultComboBoxModel fReplaceHistory;
	private int fHistorySize;
	private ReplaceAction fReplaceAct;
