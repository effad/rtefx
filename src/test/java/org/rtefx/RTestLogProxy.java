...
public class RTestLogProxy implements InvocationHandler  {
	public static Object newInstance(Object obj, RTestLogProxy proxy) {
		return java.lang.reflect.Proxy.newProxyInstance(obj.getClass().getClassLoader(), obj.getClass().getInterfaces(), proxy);
	}
...
//			System.err.println("Got an ignorable method from " + m.getDeclaringClass().getName());
		...
		if (m.getName().equals("equals") && args.length > 0 && args[0] instanceof Proxy) {
			InvocationHandler h = Proxy.getInvocationHandler((Proxy) args[0]);
	...
	private Object fObj;
	private HashSet fClasses;
	private HashSet fMethodIgnores;
	private long fTime;
}
