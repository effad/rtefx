...
public class RTestREDEditorMisc extends RTestREDEditor {
	...
	private String getClipboardAsString() throws Exception {
		...
		return "" + c.getContents(this).getTransferData(DataFlavor.stringFlavor);
	}
	...
	private void sendKey(REDEditor editor) {
	...
	public void testControllerDecoratorManagement() {
		...
		sendKey(editor);
		assertEquals("CBA", "" + log); log.delete(0, log.length());

		editor.removeControllerDecorator(b);
		sendKey(editor);
		assertEquals("CA", "" + log); log.delete(0, log.length());
		
		editor.removeControllerDecorator(a);
		sendKey(editor);
		assertEquals("C", "" + log); log.delete(0, log.length());
	...
	class TestPlugin1 extends REDPlugin {
