...
package org.rtefx;	// this test case is in package org.rtefx becaus it accesses REDText
...
//			System.out.println("Delete from " + start + " to " + end);
	...
	class PosPrinter implements REDDistanceTreeIterator {
	...
	class ToStringPrinter implements REDDistanceTreeIterator {
		...
		public void processNode(REDNode node, REDEdgeData data, int depth) {
			REDLineTreeData myData = (REDLineTreeData) data;
			fBuffer.append(" " + myData.getLine() + "," + myData.getPosition());
		}		
	...
	class TreePrinter implements REDDistanceTreeIterator {
		public void processNode(REDNode node, REDEdgeData data, int depth) {
			REDLineTreeData myData = (REDLineTreeData) data;
			...
			buf.append("-(" + myData.getLine() + ", " + myData.getPosition() + ")");
			System.out.println(buf.toString());
		}		
	}
	
	class PosCheckerPrinter implements REDDistanceTreeIterator {
		public PosCheckerPrinter (String text) {
			fLineStart = new ArrayList(); fLineStart.add(new Integer(0));
			int idx = text.indexOf("\n");
			while (idx != -1) {
				fLineStart.add(new Integer(idx+1));
				idx = text.indexOf("\n", idx+1);
			}
			fCurLine = 0;
		}
		...
		public void processNode(REDNode node, REDEdgeData data, int depth) {
			...
			assertTrue("Node: " + node + " is wrong. Expected its position to be " + ((Integer) fLineStart.get(fCurLine)).intValue() + " but it was " + myData.getPosition(), myData.getPosition() == ((Integer) fLineStart.get(fCurLine)).intValue());
			fCurLine++;
		}		
