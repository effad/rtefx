...
public class RTestREDBracketMatcherDefinition extends RTestLogObserver {
	...
	private String flattenPatterns(REDBracketMatcherDefinition def) {
	...
	private REDBracketMatcherDefinitionStyleSet getStyleSet(REDBracketMatcherDefinition def, int patNr) {
		return ((REDBracketMatcherDefinitionPattern) def.fPatterns.get(patNr)).fStyleSet;
	}
	
	private REDBracketMatcherDefinitionRuleSet getRuleSet(REDBracketMatcherDefinition def, int patNr) {
	...
	private void readAdditionalMatcherDefs(String pattern) {
		...
		while (iter.hasNext()) {
			...
			REDBracketMatcherManager.readMatcherDefinition(is, ""+iter.curName());
		}
	}
