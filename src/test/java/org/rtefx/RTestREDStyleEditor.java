...
public class RTestREDStyleEditor extends TestCase {
	...
	private void assertSortedChildren(TreeNode node) {
		TreeNode child;
		String lastNode = "";
		for (int x = 0; x < node.getChildCount(); x++) {
			child = node.getChildAt(x);
			assertTrue("Last node '" + lastNode + "' is not less than '" + child + "'.", lastNode.compareTo("" + child) < 0);
			lastNode = "" + child;
	...
	public void testIconsForTree() {
		assertNotNull(ClassLoader.getSystemResourceAsStream("config/icons/tree_collapsed.png"));
		assertNotNull(ClassLoader.getSystemResourceAsStream("config/icons/tree_expanded.png"));
	}
	
	class Listener implements REDStyleEditorSelectionListener {
	...
	void checkEvents(String expLog, RTestLogProxy proxy) {
		assertEquals("View Listener", expLog, "" + proxy);
	}
	...
	public void testSelectionToEditUpdate() {
		...
		for (int x = 0; x < tree.getRowCount(); x++) {
			...
			assertEquals(s.getFontFace(), "" + fStyleEditor.fEditFontFace.getSelectedItem());
			assertEquals("" + s.getFontSize(), fStyleEditor.fEditFontSize.getEditor().getItem());
			assertTrue(s.getFontStyle().equalsIgnoreCase("" + fStyleEditor.fEditFontStyle.getSelectedItem()));
...
//	}
	...
	public void doTestInheritedCheckboxes(String theme) {
		...
		// Lining
		assertEquals("singleunder", "" + fStyleEditor.fEditLining.getSelectedItem());
		...
		assertEquals("none", "" + fStyleEditor.fEditLining.getSelectedItem());
		assertTrue(!s.definesLining(theme));
...
"</REDConfig>\n"
	...
	private void assertFileContent(String content) throws FileNotFoundException, IOException {
